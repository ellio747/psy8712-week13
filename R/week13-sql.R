...
dbGetQuery(con, "
           SELECT city, COUNT(employee_id) AS location_n
           FROM datascience_employees
           INNER JOIN datascience_testscores
           USING (employee_id)
...
dbGetQuery(con, "
           SELECT 
             performance_group,
             ROUND(AVG(yrs_employed), 2) AS group_mean,
             ROUND(STDDEV(yrs_employed), 2) AS group_sd
           FROM datascience_employees
           INNER JOIN datascience_testscores
             USING (employee_id)
...
## uses brief table markers = e, t, o, to classify the tables for joins
dbGetQuery(con, "
           SELECT 
             office_type,
             employee_id,
           ...
           INNER JOIN datascience_testscores t
             USING (employee_id)
           INNER JOIN datascience_offices o
             ON e.city = o.office
           WHERE manager_hire = 'Y'
